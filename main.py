...
try:
    ...
    print("Install pyperclip for better experience")
    pass

commandBlock_commandWorkpiece = """summon minecraft:falling_block ~ ~1 ~ {BlockState:{Name:"redstone_block"},Time:1,Passengers:[{id:armor_stand,Health:0,Passengers:[{id:falling_block,BlockState:{Name:"activator_rail"},Time:1,Passengers:[replace77k46gob8cCH%eQNwcc8Zk9VxwReplace,{id:command_block_minecart,Command:'setblock ~ ~1 ~ command_block{auto:1,Command:"kill @e[type=command_block_minecart,distance=..3]"}'},{id:command_block_minecart,Command:'setblock ~ ~2 ~ command_block{auto:1,Command:"fill ~ ~ ~ ~ ~-3 ~ air"}'}]}]}]}"""
command_workpiece = "{id:command_block_minecart,Command:'replace77k46gob8cCH%eQNwcc8Zk9VxwReplace'}"
rep = 'replace77k46gob8cCH%eQNwcc8Zk9VxwReplace'
commands = []
...
while 1:
    tmp = input('Enter command (empty for end): ')
    if tmp != '':
